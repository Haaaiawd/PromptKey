...
impl HotkeyManager {
    ...
    pub fn register(
        &self,
        hotkey: &str,
    ) -> StdResult<(), Box<dyn std::error::Error + Send + 'static>> {
        ...
        if result.is_err() {
            ...
            log::error!(
                "Failed to register hotkey '{}'. Error code: {:?}",
                hotkey,
                error_code
            );
            ...
            log::debug!(
                "Parsed fallback hotkey: vk={:x}, modifiers={:x}",
                fallback_vk,
            ...
            if fallback_result.is_err() {
                ...
                log::error!(
                    "Failed to register fallback hotkey. Error code: {:?}",
                    fallback_error_code
                );
                ...
                return Err(Box::new(std::io::Error::new(
                    ...
                    format!(
                        "Failed to register hotkey. Primary error: {}, Fallback error: {}",
    ...
    fn parse_hotkey(
        hotkey: &str,
    ) -> StdResult<(u16, HOT_KEY_MODIFIERS), Box<dyn std::error::Error + Send + 'static>> {
        ...
        log::debug!(
            "Hotkey parsing completed: vk={:x}, modifiers={:x}",
            vk,
            modifiers.0
        );
        Ok((vk, modifiers))
...
impl HotkeyService {
    ...
    pub fn start(&mut self) -> StdResult<(), Box<dyn std::error::Error + Send + 'static>> {
        ...
        let thread_handle = std::thread::spawn(
            move || -> StdResult<(), Box<dyn std::error::Error + Send + 'static>> {
                ...
                // T1-008: Register selector hotkey (Ctrl+Shift+H, ID=3)
                let selector_hotkey = "Ctrl+Shift+H";
                let (selector_vk, selector_modifiers) =
                    HotkeyManager::parse_hotkey(selector_hotkey)?;
                unsafe {
                    let _ = UnregisterHotKey(None, 3);
                }
                let selector_result =
                    unsafe { RegisterHotKey(None, 3, selector_modifiers, selector_vk as u32) };
                if selector_result.is_ok() {
                    log::info!(
                        "Successfully registered selector hotkey: {}",
                        selector_hotkey
                    );
                } else {
                    log::warn!("Failed to register selector hotkey (non-critical)");
                }
                    ...
                    .and_then(|p| std::fs::read_to_string(&p).ok().map(|s| (p, s)))
                {
                    Some((path, content)) => {
                        if let Ok(mut cfg) = serde_yaml::from_str::<Config>(&content) {
                            ...
                            if hotkey.to_lowercase().contains("+win+")
                                || hotkey.to_lowercase().ends_with("+win")
                                || hotkey.to_lowercase().starts_with("win+")
                            {
                            ...
                            if let Ok(yaml) = serde_yaml::to_string(&cfg) {
                                if let Err(err) = std::fs::write(&path, yaml) {
                                    log::warn!(
                                        "Failed to persist effective hotkey to config: {}",
                                        err
                                    );
                                } else {
                                    log::info!(
                                        "Persisted effective hotkey to config: {}",
                                        cfg.hotkey
                                    );
                                }
                            }
                ...
                loop {
                    ...
                    // 检查是否是我们的热键消息（ID=1/2为注入热键，ID=3为选择器热键）
    ...
    pub fn stop(&mut self) {
        ...
        unsafe {
            let _ = UnregisterHotKey(None, 1);
            let _ = UnregisterHotKey(None, 2);
            let _ = UnregisterHotKey(None, 3); // T1-008: Selector hotkey
        }
